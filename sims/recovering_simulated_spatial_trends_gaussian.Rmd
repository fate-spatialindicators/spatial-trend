...
sub = dplyr::filter(grid, sigma_E==0.01)
g1 = ggplot(sub, 
  aes(phi, bias, group=phi)) + geom_hline(aes(yintercept=0),col="grey") + geom_violin(fill=col,col=NA,alpha=0.5) + xlab(expression(paste("Obs error ", sigma))) + coord_flip() +
  ylab(" ") + 
  stat_summary(fun.y=median, geom="point", size=2, color=col) + theme_sleek()
  #ylab(expression(paste("Bias E[", theta, "-",hat(theta),"]"))) +

g2 = ggplot(sub, 
  aes(phi, sd, group=phi)) + geom_violin(fill=col,col=NA,alpha=0.5) + xlab(" ") + coord_flip() + 
...
g4 = ggplot(sub, 
  aes(phi, rho, group=phi)) + geom_violin(fill=col,col=NA,alpha=0.5) + xlab(" ") + 
  ylab(" ") + stat_summary(fun.y=median, geom="point", size=2, color=col) + theme_sleek() + coord_flip()

grid = readRDS("sims/grid.rds")
sub = dplyr::filter(grid, phi==0.01)
f1 = ggplot(sub, 
  aes(sigma_E, bias, group=sigma_E)) + geom_hline(aes(yintercept=0),col="grey") + geom_violin(fill=col,col=NA,alpha=0.5) + xlab(expression(paste("Spatiotemporal ", sigma))) + 
  ylab(expression(paste("Bias E[", theta, "-",hat(theta),"]"))) + stat_summary(fun.y=median, geom="point", size=2, color=col) + theme_sleek()
f2 = ggplot(sub, 
  aes(sigma_E, sd, group=sigma_E)) + geom_violin(fill=col,col=NA,alpha=0.5) + xlab(expression(paste("Spatiotemporal ", sigma))) + 
  ylab(expression(paste("SD[", theta, "-",hat(theta),"]"))) + stat_summary(fun.y=median, geom="point", size=2, color=col) + theme_sleek()
#f3 = ggplot(sub, 
...
f4 = ggplot(sub, 
  aes(sigma_E, rho, group=sigma_E)) + geom_violin(fill=col,col=NA,alpha=0.5) + xlab(expression(paste("Spatiotemporal ", sigma))) + 
  ylab(expression(paste("Correlation (", theta, ", ",hat(theta),")"))) + stat_summary(fun.y=median, geom="point", size=2, color=col) + theme_sleek()

pdf("sims/results_fig_gaussian.pdf")
